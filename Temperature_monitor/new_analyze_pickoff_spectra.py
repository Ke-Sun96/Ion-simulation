...
folder = 'spectrum_analyzer_data20200313WarmSmallToroid470pF_WithAmp_PCBTemp/'
_format = '%Y%m%d_%H%M%S.npy'

#temperature data
filename = '20200313WarmSmallToroid470pF_WithAmp_PCBTemp.csv'
startdate = '20200313'
...
for i in range(len(data_list)):
    ...
    freqs.append(xdat[ydat==np.max(ydat)][0]) #find minimum of reflection dip, record freq
...
maxs_sorted = np.array(maxs_sorted)
...
max_avg = np.zeros(int(temp_dt[-1]/avg_int))
...
for i in range(0,int(temp_dt[-1]/avg_int)):
    ...
    max_avg[i] = np.average(maxs_sorted[np.where(np.logical_and(dt > avg_int*i, dt < avg_int*(i+1)))][0])
    freq_avg[i] = np.average(freqs_sorted[np.where(np.logical_and(dt > avg_int*i, dt < avg_int*(i+1)))][0])

plt.subplot(3,1,1)
plt.plot(temp_avg,max_avg, '.')
plt.ylabel('Pickoff peak (dBm)')
